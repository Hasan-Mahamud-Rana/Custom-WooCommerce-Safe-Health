...
class WC_Product extends WC_Abstract_Legacy_Product {
	...
	*/
	...
	public function get_price( $context = 'view' ) {
		...
		$user_specific_price = $current_user_role . "_price";
		if( strcmp($productType, "simple") == 0
			...
			$new_price = get_field( $user_specific_price, $this->get_id() );
			if( $new_price) {
			  $get_price = $new_price;
			} else {
				$get_price = $this->get_prop( 'price', $context );
			}
		} elseif( strcmp($productType, "variation") == 0
			&& ( strcmp($current_user_role, "danmark_forhandler") == 0
			|| strcmp($current_user_role, "sverige_forhandler") == 0
			|| strcmp($current_user_role, "norge_forhandler") == 0
			|| strcmp($current_user_role, "danmark_helsekost") == 0)) {

		$variable_user_specific_price = "_" . $current_user_role . "_price";
		$variations[$variable_user_specific_price] = get_post_meta( $this->get_id(), $variable_user_specific_price, true );
			if( $variations[$variable_user_specific_price] ) {
				$get_price = $variations[$variable_user_specific_price];
			} else {
				$get_price = $this->get_prop( 'price', $context );
			}
	...
	public function get_regular_price( $context = 'view' ) {
		...
		$get_regular_price = $this->get_prop( 'regular_price', $context );
