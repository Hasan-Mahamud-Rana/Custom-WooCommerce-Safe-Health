...
function wc_format_sale_price( $regular_price, $sale_price ) {
	...
	$price = '<del>Normal pris: ' . ( is_numeric( $regular_price ) ? wc_price( $regular_price ) : $regular_price ) . '</del> <ins>' . $user_specific_sale_price_label . '' . ( is_numeric( $sale_price ) ? wc_price( $sale_price ) : $sale_price ) . '</ins>';

	$discountPrice = (100 - (($sale_price / $regular_price) * 100));
	if ($discountPrice != 0){
		$discountPrice = round($discountPrice, 2);
		$price .='<span class="discount">' . $discountPrice . '% rabat</span>';
	}
